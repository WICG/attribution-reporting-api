...
Note that this technique can be composed with pre-sharing an output domain. In that case, we would sample fake records from the raw records _and_ the output domain, and we would still drop records not included in the output domain at the end. This would allow us to continue avoiding the thresholding step at the end of the protocol.

If no output domain is pre-shared, this technique can be used on its own. In that case however we would need to protect the presence / absence of any aggkey in the aggregate output. To do this we will need to re-introduce the thresholding step on the count of real records + fake records, though this technique will need some investigation:
